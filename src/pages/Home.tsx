...
import { LangCode, getLocaleText } from "../utils/I18n";
...
export default function Home({ lang }: { lang: LangCode }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const dataPromise = fetch("https://pustot.github.io/db/mcpdict.db", {
          mode: "cors",
...
function getRandom3500Han() {
  ...
  else return hansSecond1000.charAt(Math.floor(Math.random() * 1000));
}
        ...
        if (/[a-zA-Z0-9]/.test(ch)) {
          ...
          if (showGuangyunOnly) {
            for (let yitiCh of [ch, ...Yitizi.get(ch)])
              if (Qieyun.資料.query字頭(yitiCh).length != 0) terms.push(yitiCh);
      ...
      for (let item of [...new Set(terms)]) {
        ...
        if (newRes.length) {
          newRes[0].values = newRes[0].values.map((entry, i) => {
            ...
            const 音韻地位列表 = Qieyun.資料
              .query字頭(han)
              .map((v, i) => v.音韻地位);
            entry.unshift(音韻地位列表.map((v, i) => v.描述).join(","));
            entry.unshift(音韻地位列表.map((v, i) => " ").join(","));
